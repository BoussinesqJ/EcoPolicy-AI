...
def run_fetch(config: dict, region: str = None, industry: str = None):
    ...
    for i, source in enumerate(enabled_sources):
        ...
        try:
            ...
            logger.info(f"  {source_name}: 抓取 {len(all_policies)} 条，新增 {new_count} 条，相关 {len(new_in_this_source)} 条")
    ...
    logger.info(f"数据源统计: 成功 {success_count} 个，失败 {len(failed_sources)} 个")
    if failed_sources:
        logger.info(f"失败数据源: {', '.join(failed_sources)}")
