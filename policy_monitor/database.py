...
class PolicyDatabase:
    ...
    def get_recent_policies(self, days: int = 7) -> list[dict]:
        """获取最近 N 天的政策（按评分降序）"""
        from datetime import datetime, timedelta
        cutoff = (datetime.now() - timedelta(days=days)).strftime("%Y-%m-%d")
        rows = self.conn.execute(
            "SELECT * FROM policies WHERE date >= ? OR fetched_at >= ? ORDER BY score DESC",
            (cutoff, cutoff),
        ).fetchall()
        # 解析 JSON 字段
        results = []
        for r in rows:
            d = dict(r)
            if isinstance(d.get("keywords_matched"), str):
                try:
